    ...
    def get_telemetry(self) -> Optional[Dict[str, float]]:
        ...
        try:
            msg = self.master.recv_match(
                type=['GLOBAL_POSITION_INT', 'ATTITUDE', 'SYS_STATUS', 'VFR_HUD'], blocking=True, timeout=5)
            if msg:
                telemetry = {}
                if msg.get_type() == 'VFR_HUD':
                    if telemetry['speed'] == 'high':
                        logger.info("скорость высокая")
                    else:
                        logger.info("скорость низкая")
                if msg.get_type() == 'SYS_STATUS':
                    if telemetry['battery'] == 'low':
                        logger.warning("Батарея разряжяена, вернитесь на базу")
                    else:
                        logger.info("Заряд батареи достаточен")
